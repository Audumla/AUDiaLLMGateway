...
services:
  gateway:
    image: audumla/audia-llm-gateway-orchestrator:latest
  ...
  backend-swappable:
    image: audumla/audia-llm-gateway-server:latest
...
| `VLLM_IMAGE` | No | `audumla/audia-llm-gateway-vllm:latest` | Override the vLLM image. The default wraps upstream vLLM with AUDia's generated-config entrypoint. |
...
```bash
docker volume rm audumla_backend-runtime
docker compose up -d
```
