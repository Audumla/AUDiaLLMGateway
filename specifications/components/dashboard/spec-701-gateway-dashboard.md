...
```yaml
# docker/compose/docker-compose.yml addition
audia-dashboard:
  build:
    context: .
...
- [ ] Update `docker/compose/docker-compose.yml` with `dashboard` profile
...
| Docker image | `docker/compose/docker-compose.yml` | Dashboard does NOT define |
...
- [ ] Dashboard makes NO changes to `docker/compose/docker-compose.yml` (only reads)
...
# ── Hardware Correlation (Layered Sidecar)
# This section is NOT manually edited. It is automatically generated by 
# config_loader.py into config/generated/monitoring/correlation.yaml
# to prevent polluting core LLM configs.
correlations:
  - component_id: ollama
    device_class: gpu
...
Metrics are correlated with the specific GPU in Grafana via the automated sidecar join.
